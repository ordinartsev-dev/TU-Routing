...
namespace Backend.Services
{
    public class GraphHopperService
    {
        ...
        public async Task<(CarRoute path, string finalResponse)> GetCarRouteAsync(double fromLat, double fromLon, double toLat, double toLon)
        //public async Task<WalkingRoute> GetRouteAsync(double fromLat, double fromLon, double toLat, double toLon)
        {
            try
            {
                string url = $"http://localhost:8989/route?point={fromLat},{fromLon}&point={toLat},{toLon}&vehicle=car&locale=ru&instructions=true";
                ...
                var path = JsonSerializer.Deserialize<CarRoute>(jsonResponse, options);
                if (path != null)
                {
                    Console.WriteLine("Route found! Route length:" + path.PrintLength());
                    // Decode the route points
                    ...
                    RouteResponce carRouting = new RouteResponce
                    {
                        Type = "Car",
                        Start = new List<double> { decodedPoints[0].Latitude, decodedPoints[0].Longitude },
                        End = new List<double> { decodedPoints[decodedPoints.Count - 1].Latitude, decodedPoints[decodedPoints.Count - 1].Longitude },
                    ...
                    finalResponse = JsonSerializer.Serialize(carRouting);
